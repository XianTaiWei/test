...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    let homeNav = UINavigationController(rootViewController: FirstViewController())
    let lookNav = UINavigationController(rootViewController: SecondViewController())

    lookNav.tabBarItem.title = "发现"
    homeNav.tabBarItem.title = "我"
    
    

    lookNav.tabBarItem.image = UIImage(named: "found")
    homeNav.tabBarItem.image = UIImage(named: "home")
    
    
    let tabBarController = UITabBarController()
    tabBarController.viewControllers = [homeNav,lookNav]
